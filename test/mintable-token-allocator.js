...
contract("MintableTokenAllocator", accounts => {
    ...
    describe("MintableTokenAllocator", () => {
        ...
        it("should not allow to allocate from not crowdsale", async () => {
            ...
            const token = await MintableToken.new(management.address);
            const allocator = await MintableTokenAllocator.new(1000000, management.address);
    ...
    describe("MintableToken", () => {
        it("available tokens should return 900", async () => {
            const holder = accounts[1];
            const crowdsale = accounts[2];
            ...
            const res = await allocator.tokensAvailable.call(await token.totalSupply.call());
        ...
        it("should not allow to allocate because totalSupply == maxSupply", async () => {
            ...
            const allocator = await MintableTokenAllocator.new(new BigNumber(1000000).toString(), management.address);
            ...
            await allocator.allocate(holder, new BigNumber(1000000).toString(), 0, {from: crowdsale})
                .then(Utils.receiptShouldSucceed);
            ...
            await assert.equal(
                new BigNumber(currentBalance).valueOf(),
                new BigNumber(1000000).toString(),
                "currentBalance is not equal"
            );

            await allocator.allocate(holder, new BigNumber(1000000).toString(), new BigNumber(1000000).toString(), {from: crowdsale})
                .then(Utils.receiptShouldFailed)
                .catch(Utils.catchReceiptShouldFailed);
        ...
        it("should allow to decrease max supply", async () => {
            const holder = accounts[1];
            const crowdsale = accounts[2];
            ...
            await allocator.decreaseCap(new BigNumber(900000).toString())
                .then(Utils.receiptShouldFailed)
                .catch(Utils.catchReceiptShouldFailed);
            ...
            await allocator.decreaseCap(new BigNumber(999800).toString(),{from: crowdsale})
                .then(Utils.receiptShouldSucceed);
            ...
            await allocator.allocate(holder, 100, 200, {from: crowdsale})
                .then(Utils.receiptShouldFailed)
                .catch(Utils.catchReceiptShouldFailed);
